...
读 `packages/core/src/config.test.ts` 复用其 `loadConfig({...})` 传 env 的写法。新增用例：
```ts
test('skillNudgeInterval 默认 10', () => {
  expect(loadConfig({ GLM_API_KEY: 'k' }).skillNudgeInterval).toBe(10);
});
test('skillNudgeInterval 读 env', () => {
  expect(loadConfig({ GLM_API_KEY: 'k', HERMES_SKILL_NUDGE_INTERVAL: '5' }).skillNudgeInterval).toBe(5);
});
test('skillNudgeInterval=0 表示关闭(不被默认覆盖)', () => {
  expect(loadConfig({ GLM_API_KEY: 'k', HERMES_SKILL_NUDGE_INTERVAL: '0' }).skillNudgeInterval).toBe(0);
});
test('skillNudgeInterval 非法值回退 10', () => {
  expect(loadConfig({ GLM_API_KEY: 'k', HERMES_SKILL_NUDGE_INTERVAL: 'abc' }).skillNudgeInterval).toBe(10);
});
```
