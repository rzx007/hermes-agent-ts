...
在 `packages/core/src/skill-store.test.ts` 末尾追加（文件顶部已有 `import { test, expect } from 'vitest'`；需补 `import { mkdtempSync, rmSync, existsSync, readFileSync } from 'node:fs'; import { tmpdir } from 'node:os'; import { join } from 'node:path';` 若未导入——先看文件现有 import，缺啥补啥）：

```ts
function tmpSkills(): string {
  return mkdtempSync(join(tmpdir(), 'hermes-skills-b-'));
}
...
追加到 `skill-store.test.ts`（复用 Task 1 的 `tmpSkills`/`SKILL` 助手）：
...
test('delete 软链接技能目录被拒（不删 symlink）', () => {
  // 见实现说明：用 symlinkSync 造一个指向技能目录的链接条目；若平台不支持创建符号链接则跳过
  const dir = tmpSkills();
  const realStore = new SkillStore(dir);
  realStore.create('real', SKILL('real'));
  // 构造一个其目录为 symlink 的技能：在另一个临时根里 symlink 到 real 的目录
  const dir2 = tmpSkills();
  const { symlinkSync } = await import('node:fs');
  let linked = true;
  try {
    symlinkSync(join(dir, 'real'), join(dir2, 'linked'), 'junction');
  } catch {
    linked = false;
  }
  if (linked) {
    const store2 = new SkillStore(dir2);
    expect(() => store2.delete('real')).toThrow(/symlink|链接/i);
  }
  rmSync(dir, { recursive: true, force: true });
  rmSync(dir2, { recursive: true, force: true });
});
```
> 注：该 symlink 测试用 `async () =>` 测试函数（`test('...', async () => {...})`）。若你担心平台差异，保留 `linked` 守卫即可——不支持时该用例不做断言但仍通过。根目录删除一路由 `delete()` 内部断言保护，难以从公开 API 触发（技能目录至少深一层），故不单独写公开用例，靠断言代码 + 评审覆盖。
