...
| 文件 | 改动 |
| --- | --- |
| `packages/core/src/skill-store.ts` | `SkillEntry` 增 `file` 字段；新增 `create/edit/patch/delete` 方法 + 校验纯函数 + 原子写 |
...
| `packages/tools/src/builtin/index.ts` | 注册 `skill_manage` |
...
`SkillEntry` 增加 `file: string`（SKILL.md 绝对路径），供 edit/patch/delete 定位文件。

四个变更方法，统一流程：**校验 → 原子写（tmp + rename）→ 同步内存 `byName` / `skills`**。校验失败 `throw Error`（工具层捕获回灌模型自纠）。
...
- **`create(name, content, category?)`**
  ...
  - 目录：`root/[category/]name/`，`mkdir -p` 后原子写 `SKILL.md`。
  - 解析入 `skills` / `byName`。返回写入路径。

- **`edit(name, fullContent)`**
  - 须存在（`byName`）。校验 fullContent 的 frontmatter / 正文 / 大小。
  - **不允许改 frontmatter 的 `name`**（改名 = delete + create）；frontmatter.name ≠ 现 name → 报错。
  - 原子重写**同一路径**（category 由路径决定，保持不变）；重解析更新条目（content + description）。
...
- **`patch(name, oldString, newString, replaceAll?)`**
  ...
  - 校验改后整体仍是合法 SKILL.md（frontmatter 必字段、正文非空、大小）。
  - 原子写 + 重解析更新条目。
...
```ts
schema: {
  action: enum['create','edit','patch','delete'],
  name: string,
  content?: string,      // create/edit 必填
  old_string?: string,   // patch 必填
  new_string?: string,   // patch 必填
  replace_all?: boolean, // patch 可选，默认 false
  category?: string,     // create 可选
}
```
handler：
...
## 测试
- SkillStore：create（成功/重名/非法名/frontmatter.name 不一致/category 非法/正文空/超大）；edit（成功/不存在/改 name 被拒）；patch（唯一替换/未找到/不唯一/replaceAll/`$` 安全/改后 frontmatter 失效被拒）；delete（成功/不存在/根目录拒/symlink 拒）；热更新可见性（create 后 list/getContent/renderIndex 立即含）。
